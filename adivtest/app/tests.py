...
class test_OrderNewItemASM(TestCase):
...
class test_OrderExisting_item(TestCase):
    def test_if_order_submitted(self):
        ordering_existing_items_request({'camera':2})
        order=db.child('orders').child('user_id').get()
        self.assertIsNotNone(order)

class test_manage_order_Wmanager(TestCase):
    def test_IfTableExist(self):
        orders=db.child('orders').get()
        flag=0
        for i in orders.each():
            if db.child('orders').child(i.key()).child('role').get().val()==3:
                flag+=1
        if(flag==0):
            self.assertIsNone(manage_orders())
        else:
            self.assertIsNotNone(manage_orders())

    # def test_if_order_approved(self):
    #     orders=db.child('orders').get()
    #     flag=0
    #     for i in orders.each():
    #         if db.child('orders').child(i.key()).child('role').get().val()==3:
